...
def _strip_init_references(root: Path, config: ProjectConfig) -> None:
    """Remove init.sh references from various files.
...
def _strip_init_refs_from_docs(root: Path) -> None:
    """Remove init.sh references from docs and workflow files.
    ...
    if pub.exists():
        replace_in_file(
            pub,
            'run `init.sh` with `--pypi`, or manually',
            'manually',
        )
        replace_in_file(
            pub,
            'has been updated by `init.sh` with',
            'contains',
        )
    ...
    if release.exists():
        replace_in_file(
            release,
            'Run init.sh with --pypi, or uncomment',
            'Uncomment',
        )


def _strip_init_refs_from_md(root: Path, spdx: str) -> None:
    """Remove init.sh references from README and CLAUDE.md.
    ...
    if readme.exists():
        replace_in_file(
            readme,
            'Run `./scripts/init.sh --pypi` to enable publishing '
            '(or uncomment',
            'Uncomment',
        )
        replace_in_file(
        ...
        replace_in_file(
            readme,
            'Apache-2.0 license (configurable via init.sh)',
            f'{spdx} license',
        )
...
def _update_code_style_rules(root: Path, config: ProjectConfig) -> None:
    ...
    old_section = (
        '## License Headers\n'
        '\n'
        '- After running `init.sh` with a license selection, '
        'all `.py` files\n'
